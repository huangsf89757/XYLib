...
// MARK: - XYCentralManagerWrapper
/// 蓝牙中心管理器类，封装了CBCentralManager的功能，提供蓝牙设备扫描、连接等操作
open class XYCentralManagerWrapper: NSObject {
    // MARK: var
    /// CBCentralManager实例，用于管理蓝牙中心设备
    public private(set) var centralManager: CBCentralManager!
    /// 代理对象，用于处理蓝牙事件回调
    public weak var delegate: (any XYCentralManagerWrapperDelegate)?
    ...
    public init(delegate: (any XYCentralManagerWrapperDelegate)?, queue: dispatch_queue_t? = nil, options: [String : Any]? = nil) {
        super.init()
    ...
    open func stopScan() {
        ...
        XYBleLog.debug()
        self.delegate?.centralManagerStopScan(centralManager)
    }
    ...
    open func registerForConnectionEvents(options: [CBConnectionEventMatchingOption : Any]? = nil) {
        ...
        self.delegate?.centralManager(centralManager, registerForConnectionEventsWith: options)
    }
}

// MARK: - CBCentralManagerDelegate
extension XYCentralManagerWrapper: CBCentralManagerDelegate {
