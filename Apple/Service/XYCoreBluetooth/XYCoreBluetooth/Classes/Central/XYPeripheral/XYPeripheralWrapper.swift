...
// MARK: - XYPeripheralWrapper
/// 蓝牙外设包装类，封装了CBPeripheral的功能，提供服务发现、特征读写等操作
open class XYPeripheralWrapper: NSObject {
    // MARK: var
    /// CBPeripheral实例，系统蓝牙外设对象
    public private(set) var peripheral: CBPeripheral!
    /// 代理
    public weak var delegate: (any XYPeripheralWrapperDelegate)?
    ...
    public init(peripheral: CBPeripheral, delegate: (any XYPeripheralWrapperDelegate)? = nil) {
        super.init()
    ...
    open func readRSSI() {
        peripheral.readRSSI()
        XYBleLog.debug()
        self.delegate?.peripheralReadRSSI(peripheral)
    }
...
extension XYPeripheralWrapper: CBPeripheralDelegate {
    /// 外设名称更新回调
...
// MARK: - Func
extension XYPeripheralWrapper {
    /// 获取指定特征
    public func getCharacteristic(uuidString: String) -> CBCharacteristic? {
        let services = peripheral.services ?? []
        if let characteristic = _getCharacteristic(uuidString: uuidString, in: services) {
            return characteristic
        }
        for service in services {
            let includedServices = service.includedServices ?? []
            if let characteristic = _getCharacteristic(uuidString: uuidString, in: includedServices) {
                return characteristic
            }
        }
        return nil
    }
    private func _getCharacteristic(uuidString: String, in services: [CBService]) -> CBCharacteristic? {
        for service in services {
