...
public protocol XYCentralManagerWrapperDelegate: CBCentralManagerDelegate {
    // MARK: Retrieve
    /// 检索外设通知
    /// - Parameters:
    ///   - central: 中心管理器实例
    ...
    ///   - central: 中心管理器实例
    ///   - _: 空参数
    func centralManagerStopScan(_ central: CBCentralManager)
    ...
    ///   - options: 注册选项
    @available(iOS 13.0, *)
    func centralManager(_ central: CBCentralManager, registerForConnectionEventsWith options: [CBConnectionEventMatchingOption : Any]?)
}

public extension XYCentralManagerWrapperDelegate {
    @available(iOS 7.0, *)
    func centralManager(_ central: CBCentralManager, retrievePeripheralsWithIdentifiers identifiers: [UUID], returns peripherals: [CBPeripheral]) {}
